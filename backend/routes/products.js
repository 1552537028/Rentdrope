...
router.post('/', upload.array('images', 8), async (req, res) => {
  try {
    ...
    const images = req.files.map(file => file.filename); // Store only the filename without the 'uploads/' prefix
...
router.get('/', async (req, res) => {
  try {
    ...
    // Map through each product and prepend the correct base URL to the image filenames
    const productsWithImageUrls = products.map(product => ({
      ...product.toObject(),
      images: product.images.map(image => `${process.env.BASE_URL || 'http://localhost:5000'}/uploads/${image}`)
    }));
